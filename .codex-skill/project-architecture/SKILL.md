...
| `agents` | `AgentConfig` | Agent 配置 (model_name/system_prompt/temperature/enabled) |
| `threads` | `Thread` | 会话 (user_id/agent_id/title) |
...
| GET | `/auth/me` | 当前用户 | 是 |
| GET | `/agents` | Agent 列表 | 是 |
| POST | `/agents` | 创建 Agent | 是 |
| PATCH | `/agents/{id}` | 更新 Agent | 是 |
| DELETE | `/agents/{id}` | 删除 Agent | 是 |
| GET | `/threads` | 会话列表 | 是 |
| POST | `/threads` | 创建会话 | 是 |
| GET | `/threads/{id}/messages` | 消息列表 | 是 |
| POST | `/chat` | 发送对话 | 是 |
| GET | `/mcp-servers` | MCP 列表 | 是 |
| POST | `/mcp-servers` | 创建 MCP | 是 |
| PATCH | `/mcp-servers/{id}` | 更新 MCP | 是 |
| DELETE | `/mcp-servers/{id}` | 删除 MCP | 是 |
| POST | `/mcp-servers/{id}/test` | 测试 MCP | 是 |
| GET | `/skills` | Skill 列表 | 是 |
| POST | `/skills` | 创建 Skill | 是 |
| PATCH | `/skills/{id}` | 更新 Skill | 是 |
| DELETE | `/skills/{id}` | 删除 Skill | 是 |

## 前端架构

单页 React 应用 (`web/src/main.jsx`)，通过 `view` 状态切换四个视图：
...
1. **修改 `app/models.py`** — 新增 SQLAlchemy ORM Model 或在现有 Model 加 `mapped_column`
2. **修改 `app/schemas.py`** — 新增对应的 Pydantic Create/Read/Update schema
3. **修改 `app/api.py`** — 新增 RESTful 路由 (CRUD)
4. **更新 `schema/mysql_create_tables.sql`** — 同步 MySQL DDL
5. **更新前端 `web/src/main.jsx`** — 新增页面/组件/API 调用
6. **运行 `init_db()`** — 自动建表 (首次启动时 `app/server.py` startup 事件触发)
